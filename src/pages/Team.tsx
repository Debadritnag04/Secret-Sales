...
import { Shield, TrendingUp, Users, AlertCircle, Download, Loader2 } from 'lucide-react';
...
function formatCredits(val: number): string {
  ...
  return rounded % 1 === 0 ? `${rounded} Cr` : `${rounded.toFixed(1)} Cr`;
}
...
export default function Team() {
  ...
  const handleDownloadPDF = useCallback(async () => {
    ...
    try {
      ...
      doc.text('SECRET AUCTION', pageWidth / 2, 20, { align: 'center' });
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      doc.text('SQUAD REPORT', pageWidth / 2, 28, { align: 'center' });

      // Room / Auction info
      doc.setFontSize(10);
      doc.text(`Auction: ${room?.auctionName || 'Auction'}`, 14, 40);
      doc.text(`Room: ${room?.roomCode || ''}`, 14, 46);
      doc.text(`Squad: ${mySquad.squadName}`, 14, 52);
      doc.text(`Manager: ${mySquad.ownerName}`, 14, 58);
      doc.text(`Generated: ${new Date().toLocaleString()}`, 14, 64);

      // Budget Summary
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('BUDGET SUMMARY', 14, 78);

      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      const startingBudget = room?.settings?.startingBudget || (budget + spent);
      doc.text(`Starting Budget: ${formatCredits(startingBudget)}`, 14, 86);
      doc.text(`Total Spent: ${formatCredits(spent)}`, 14, 92);
      doc.text(`Remaining: ${formatCredits(budget)}`, 14, 98);
      doc.text(`Players: ${playerCount}`, 14, 104);

      // Player Table
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text('SQUAD ROSTER', 14, 118);

      if (roster.length === 0) {
        doc.setFontSize(10);
        doc.setFont('helvetica', 'italic');
        doc.text('No players purchased yet.', 14, 126);
      } else {
        const tableData = roster.map((item: any, idx: number) => {
        ...
        autoTable(doc, {
          startY: 122,
          head: [['#', 'Player', 'Pos', 'OVR', 'Bid', 'Nationality', 'Club']],
          body: tableData,
          styles: { fontSize: 9 },
      ...
      doc.setFontSize(8);
      doc.setFont('helvetica', 'italic');
      doc.text('Generated by Secret Auction', pageWidth / 2, finalY + 15, { align: 'center' });

      // Save
      const safeName = mySquad.squadName.replace(/[^a-zA-Z0-9 ]/g, '').replace(/\s+/g, '-');
      doc.save(`Secret-Auction-${safeName}-${room?.roomCode || 'Squad'}.pdf`);
    } catch (err) {
      console.error('PDF generation failed:', err);
    } finally {
      setPdfLoading(false);
    }
  }, [room, mySquad, roster, budget, spent, playerCount]);
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={pdfLoading}
          className="flex items-center gap-2 px-5 py-3 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-xl text-white font-medium transition-colors cursor-pointer disabled:opacity-50"
        >
          {pdfLoading ? (
            <>
